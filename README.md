...
* Consolidated everything we shipped across phases (modular API, seed wizard, labels + public page, exports/imports, reports, bulk ops, concurrency/ETag, idempotency, webhooks, schema polish).
* Updated the README to be a single source of truth: architecture, directory layout, environment, commands, endpoints, formats, guardrails, and examples.
* Kept statements aligned with your actual code paths and settings (no speculative features).
* Wrote with operator-focused details (how to run, debug, reset) and API-focused contracts (payloads, CSV headers).

Planned Changes:

* PATH: `README.md` — Replace with a comprehensive, production-grade document covering the full system.
...
wizard\_seed.py     # Seed wizard ViewSet (multi-step + compose)
labels.py          # LabelViewSet (create/rotate/revoke)
batch\_ops.py       # Batch actions (harvest, cull, complete)
plant\_ops.py       # Plant bulk status updates (sell/discard/etc.)
events\_export.py   # Event export action
imports.py         # CSV import endpoints
reports.py         # Inventory & production report endpoints
public\_views.py      # Public label page (token-based HTML)
...
management/commands/
dev\_seed.py        # Idempotent developer seed
deliver\_webhooks.py# Worker for outbound webhooks
templates/
public/label\_detail.html   # Human-friendly public page for labels
nursery\_tracker/
...
Post-Checks:
- No code changes beyond `README.md`. You can open Swagger (`/api/docs/`) and run `python manage.py test` to validate docs align with the running API.
- If you added new env keys, confirm they load by restarting the dev server.

Risks & Follow-ups:
- README can drift if endpoints evolve. Treat this as the source of truth and update alongside API changes.
- When you add the Webhook Admin API (CRUD for endpoints + deliveries browse), extend the README’s Webhooks section with those routes and request/response examples.
- If you later add request-ID middleware and structured logs, add a short “Request IDs & Logging” section and header examples.
